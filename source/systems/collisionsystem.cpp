...
{
    auto viewRect = registry.view<RectCollider, Position, CollisionLayer, Velocity, Active, Gravity, Health>();
    auto viewGrid = registry.view<TileGridCollider, Position, TileGrid, CollisionLayer, Active>();
    for (auto rect_entt : viewRect)
    {

        auto &&[rect_pos, rect_collider, rect_vel, rect_layer, grav, hp] =
            viewRect.get<Position, RectCollider, Velocity, CollisionLayer, Gravity, Health>(rect_entt);

        SDL_FRect x_rect{rect_pos.position.x() + rect_vel.x * dt + rect_collider.rect.x,
                             rect_pos.position.y() + rect_collider.rect.y, rect_collider.rect.w,
                             rect_collider.rect.h};
        SDL_FRect y_rect{rect_pos.position.x() + rect_collider.rect.x,
                ...
                for (auto &row : tile_grid.cell)
                {
                    ...
                    for (auto &id : row)
                    {
                        ...
                        if (id)
                        {
                            ...
                            auto direction = AABBW(y_rect, world_tile);
                            if (direction.second != Vector2D::zero())
                            {
                                ...
                                else if (tile_layer.layer == LayersID::TRAPS)
                                {
                                    if (registry.has<Player>(rect_entt))
                                    {
                                    CloseGame();
                                    return;
                                    }
                                    else if (registry.has<Enemy>(rect_entt))
                                    {
                                    hp.health = 0;
                                    continue;
                                    }
                            ...
                            direction = AABBW(x_rect, world_tile);
                            if (direction.first != Vector2D::zero())
                            {
                                if (tile_layer.layer == LayersID::WALLS)
                                {
