...
void EnemyCreate(Vector2D spawn)
{
    ...
    registry.assign<Health>(enemy, 1);
    registry.assign<ParticleData>(enemy);
    registry.assign<Active>(enemy);
    speed.speed = 150;
    registry.assign<CollisionLayer>(enemy, LayersID::ENEMY);

    sprite.texture = textureCache.resource("batsprite");

    sprite.scale = {2, 2};
    sprite.rect = batSheet[batSheet("bats_idle").first->second];
    sprite.layer = 2;
    ...
    auto attack = batSheet.GetTypeFamily("bats_attack");
    auto idle = batSheet.GetTypeFamily("bats_idle");
    auto dead = batSheet.GetTypeFamily("bats_dead");

    animation.data.emplace("idle", Animation{idle, 0.07f, 0, 0});
    animation.data.emplace("run", Animation{attack, 0.07f, 0, 0});
    animation.data.emplace("dead", Animation{dead, 0.07f, 0, 0});
...
void EnemyCharging(const CollisionData &lhs, const CollisionData &rhs)
{
    if (registry.has<View>(lhs.id) && registry.has<Player>(rhs.id))
    {
        ...
        auto &&[enemy_pos, enemy_vel, enemy_speed, enemy, animat] =
            registry.get<Position, Velocity, MovementSpeed, Enemy, AnimationPool>(parent);
...
{
    auto view = registry.view<Enemy, Velocity, MovementSpeed, AnimationPool, Active>();
    view.each([dt](auto &enemy, auto &vel, auto &speed, auto &animat, const auto &) {
        animat.current = "idle";
