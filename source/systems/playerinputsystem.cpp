...
void PlayerMovement(const float dt)
{
    ...
    auto view = registry.view<Player, Velocity, MovementSpeed, AnimationPool, Sprite, Dash, Active, Fuel, Gravity>();
    if (view.begin() != view.end())
    {
        auto &&[vel, speed, animation, sprite, dash, fuel, grav] =
            view.get<Velocity, MovementSpeed, AnimationPool, Sprite, Dash, Fuel, Gravity>(*view.begin());
        ...
        if (state[SDL_SCANCODE_W])
        {
            if (fuel.amount > 0)
            {
                animation.current = "fly";
                vel.y += 50;
                fuel.amount -= fuel.loss * dt;
            }
            else {fuel.amount = 0; animation.current = "idle";}
        }

        if (!grav.active)
        {
            if (fuel.amount<fuel.max)
            fuel.amount += fuel.gain * dt;
            else fuel.amount = fuel.max;
        }


        if (state[SDL_SCANCODE_A])
        {
            if (animation.current != "fly")
